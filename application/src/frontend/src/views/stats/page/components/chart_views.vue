...
import {GetPageChartViews} from "@backend/api-front/routes/stats/page";
...
let pageViews: Ref<GetPageChartViews[] | undefined> = ref();
...
function fillMissingDates(chartData: GetPageChartViews[], minDate: string, maxDate: string, sites: string[], hourly: boolean): GetPageChartViews[] {
  const filledData: GetPageChartViews[] = [];
...
function getGraph(valueKey: keyof GetPageChartViews, pageViews: GetPageChartViews[], graphName: string)
{
  const dataX: string[] = [];
  const dataY: number[] = [];
