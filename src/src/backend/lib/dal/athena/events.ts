...
export class AthenaEvents extends AthenaBase {
  ...
  cteFilteredDataQuery(columns: string[], fromDate: Date, toDate: Date, sites: string[], filter?: EventFilter) {
    ...
    return `
          cte_data_filtered AS (
              SELECT ${columns.join(', ')}
              FROM events
              WHERE (${cteSiteWhereClauseSites}) AND (${cteDateWhereClauseDates})
                AND tracked_at BETWEEN parse_datetime('${exactTimeFrom}','yyyy-MM-dd HH:mm:ss.SSS')
                  AND parse_datetime('${exactTimeTo}','yyyy-MM-dd HH:mm:ss.SSS')
                ${cteWhereAndClauseExtra}
          )`;
  }

  async totalsForPeriod(fromDate: Date, toDate: Date, sites: string[], filter?: EventFilter) {
    const query = `
          WITH ${this.cteFilteredDataQuery(['site', 'category', 'event', 'data', 'user_id'], fromDate, toDate, sites, filter)},
  ...
  async chartViewsForPeriod(
    ...
    const query = `
          WITH ${this.cteFilteredDataQuery([groupBy, 'user_id', 'tracked_at', 'data'], fromDate, toDate, sites, filter)}
          SELECT
  ...
  async usersGroupedByStatForPeriod(fromDate: Date, toDate: Date, sites: string[], stat: keyof Page, filter?: EventFilter) {
  ...
  //
  // async rollupEvents(pageViewBucketName: string, site: string, date: string) {
