import {DateTime, Duration, IANAZone, Interval, Settings, ZoneOptions} from "luxon";
...
export class DateUtils
{
  ...
  {
    let tzDate = DateTime.fromJSDate(date).setZone(zone);
    /* Take the Local time at TZ and say that it is at UTC */
    let localDate = DateTime.fromObject({
      year: tzDate.year,
      month: tzDate.month,
      day: tzDate.day,
      hour: tzDate.hour,
  ...
  {
    let tzDate = DateTime.fromJSDate(date).setZone("UTC");
    /* Take the Local time at TZ and say that it is at UTC */
    let localDate = DateTime.fromObject({
      year: tzDate.year,
      month: tzDate.month,
