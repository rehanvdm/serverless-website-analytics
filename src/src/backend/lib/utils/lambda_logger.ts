...
function logLine(line: LogLine)
{
  if(process.env.TESTING_LOCAL === "true")
  {
    ...
    let timeTaken = timeDiff ? c.gray(" +" + timeDiff.toString() + "ms"): "";
    if(line.level === LogLevel.AUDIT)
    {
      let auditLog = <AuditLog>line.msg;
      if(auditLog.success)
      {
    ...
    {
      let msgWithStartMarker = c.underline((String(line.msg)).charAt(0)) + (String(line.msg)).slice(1);
      switch (line.level)
      {
        case LogLevel.DEBUG:
...
export class LambdaLog
{
  ...
  private log(logLevel: LogLevel, message: string | Error | AuditLog, optionalParams: any[])
  {
    ...
    else if(logLevel === LogLevel.AUDIT && this.errorOccurred && this.logBufferedLinesOnError) //&& !process.env.TESTING_LOCAL
    {
      logLine(line = {
        date: date,
        level: LogLevel.INFO,
