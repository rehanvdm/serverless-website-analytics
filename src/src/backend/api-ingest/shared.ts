...
{
  if(!maxMindReader)
    maxMindReader = await Reader.open( LambdaEnvironment.GEOLITE2_CITY_PATH.startsWith(".") ? path.resolve(LambdaEnvironment.GEOLITE2_CITY_PATH) : LambdaEnvironment.GEOLITE2_CITY_PATH);

  let countryIso = undefined;
