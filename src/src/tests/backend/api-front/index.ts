...
describe('API Frontend', function () {
  ...
  it('Get sites - Not Authenticated', async function () {
    this.timeout(TimeOut * 1000);
  ...
  it('Get partitions', async function () {
    this.timeout(TimeOut * 1000);
    ...
    expect(resp.statusCode).to.eq(200);
    const respData = JSON.parse(resp.body);
    expect(respData.result.data[0].site).to.be.a('string');
    expect(respData.result.data[0].year).to.be.a('number');
    expect(respData.result.data[0].month).to.be.a('number');
  });

  it('Update site indexes - Force repair - true', async function () {
    this.timeout(TimeOut * 1000);
  ...
  it('Update site indexes - Force repair - false', async function () {
    this.timeout(TimeOut * 1000);
