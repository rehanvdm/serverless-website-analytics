...
export const getSystemStore = defineStore('counter', () => {
  ...
  const cognitoLoginUrlWithRedirect = computed(() => frontendEnvironment.value.cognitoLoginUrl + '&redirect_uri=' + encodeURIComponent(window.location.href) + 'login_callback');
