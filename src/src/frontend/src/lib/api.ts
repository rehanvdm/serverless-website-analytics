...
export async function apiWrapper<T>(func: Promise<T>, loadingState?: Ref<boolean>, displayError: boolean = true) {
  const systemStore = getSystemStore();
  try {
    if (loadingState)
  ...
  {
    if(err instanceof TRPCError && err.code === 'UNAUTHORIZED' && systemStore.cognitoLoginUrlWithRedirect())
    {
    ...
    if (displayError)
      ElNotification({title: 'API Error', message: (err as any).message, type: 'error', position: "bottom-right"});
